...
const SWR_KEY = 'MySellingNfts';

const fetchMySellingNfts = async (
  ): Promise<Nft[]> => {
    ...
    const data: MarketItem[] = await contract.fetchItemsListed()
  ...
  export const useMySellingNfts = () => {
    const { data, error } = useSWR(SWR_KEY, async () => {
      return await fetchMySellingNfts();
    });
    return {
      nfts: data,
  ...
  export const useRefreshMySellingNfts = () => {
